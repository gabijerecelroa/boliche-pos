...
function App() {
  ...
  // Estados Boleteria y Admin Precios
  ...
  const handleLogin = async (e) => { 
    ...
    if (error || !data) alert('❌ Error: Usuario o Contraseña incorrectos'); 
      ...
      else if (data.rol === 'puerta') setVista('puerta'); 
  ...
  const crearStaff = async (e) => {
    e.preventDefault(); setLoading(true);
    await supabase.from('cajeros').insert([{ usuario: nuevoStaffUser, password: nuevoStaffPass, rol: nuevoStaffRol, cupo_listas: nuevoStaffRol === 'puerta' ? Number(nuevoStaffCupo) : 0 }]);
    setNuevoStaffUser(''); setNuevoStaffPass(''); setNuevoStaffCupo(0); await cargarDatos(); setLoading(false); alert('✅ Empleado creado exitosamente.');
  };

  const eliminarStaff = async (id, nombre) => {
    if (window.confirm(`¿Seguro que deseas ELIMINAR al usuario "${nombre}"?`)) {
      setLoading(true); await supabase.from('cajeros').delete().eq('id', id); await cargarDatos(); setLoading(false);
    }
  };

  const sumarCupoStaff = async (id, cupoActual, nombre) => {
    const sumar = prompt(`¿Cuántos pases VIP extras le vas a sumar a ${nombre}?`);
    if (sumar && !isNaN(sumar)) {
      await supabase.from('cajeros').update({ cupo_listas: cupoActual + Number(sumar) }).eq('id', id);
      cargarDatos();
    }
  };
  ...
  const actualizarPreciosTaquilla = async (e) => {
    ...
    await cargarDatos(); setLoading(false); alert('✅ Precios de taquilla actualizados.');
  };
  ...
  const barraHeader = (
    <header className="bg-gray-800 px-4 py-3 border-b border-gray-700 flex flex-col md:flex-row justify-between items-center mb-4 rounded-b-xl lg:rounded-xl gap-3 shadow-lg">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        ...
        {sesionActiva && (user.rol === 'admin' || user.rol === 'puerta') && vista !== 'puerta' && <button onClick={() => setVista('puerta')} className="bg-yellow-600 hover:bg-yellow-500 text-[10px] sm:text-xs px-3 py-2 rounded font-bold uppercase shadow text-black">🚪 QRs / Puerta</button>}
            ...
            <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 shadow-xl">
              ...
              <form onSubmit={crearStaff} className="space-y-3 mb-4">
                ...
                <div className="flex space-x-2">
                  <input type="text" placeholder="Contraseña" className="w-1/2 bg-gray-700 p-3 rounded-lg focus:outline-none text-sm" value={nuevoStaffPass} onChange={e => setNuevoStaffPass(e.target.value)} required />
                  <select className="w-1/2 bg-gray-700 p-3 rounded-lg focus:outline-none text-sm font-bold" value={nuevoStaffRol} onChange={e => setNuevoStaffRol(e.target.value)}>
                    <option value="cajero">Cajero (Barra)</option>
                    <option value="boleteria">Boletería</option>
                    <option value="puerta">Puerta (QR)</option>
                  </select>
                </div>
                {nuevoStaffRol === 'puerta' && (
                  <div>
                    <label className="text-xs text-yellow-400 font-bold uppercase mb-1 block">Cupo de Invitados (Listas VIP)</label>
                    <input type="number" placeholder="Ej: 50" className="w-full bg-gray-700 p-3 rounded-lg focus:outline-none text-sm font-bold" value={nuevoStaffCupo} onChange={e => setNuevoStaffCupo(e.target.value)} required />
                  </div>
                )}
                <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 py-3 rounded-lg font-black uppercase text-sm">Crear Usuario</button>
              </form>
              <div className="max-h-[150px] overflow-y-auto custom-scrollbar space-y-2">
                {staff.filter(s => s.rol !== 'admin').map(s => (
                  <div key={s.id} className="bg-gray-700 p-3 rounded-lg flex justify-between items-center text-sm">
                    <div>
                      <p className="font-bold text-white uppercase">{s.usuario}</p>
                      <p className="text-[10px] text-gray-400 uppercase">{s.rol} {s.rol === 'puerta' ? `| Cupo: ${s.cupo_listas}` : ''}</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      {s.rol === 'puerta' && <button onClick={() => sumarCupoStaff(s.id, s.cupo_listas, s.usuario)} className="text-yellow-400 bg-gray-800 px-2 py-1 rounded text-xs font-bold hover:bg-gray-600">+ Cupo</button>}
                      <button onClick={() => eliminarStaff(s.id, s.usuario)} className="text-red-500 hover:text-red-400 text-lg">❌</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {bebidas.map(b => (
                <div key={b.id} className={`p-4 rounded-xl border relative ${b.stock < 10 ? 'bg-red-900/10 border-red-900' : 'bg-gray-700/30 border-gray-600'}`}>
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-green-400 font-black text-lg">${b.precio}</span><span className={`font-bold text-xs bg-gray-800 px-2 py-1 rounded ${b.stock < 10 ? 'text-red-400' : 'text-gray-300'}`}>Stock: {b.stock}</span>
                  </div>
                  <div className="flex space-x-2">
  ...
  if (vista === 'puerta' || user.rol === 'puerta') {
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8 flex flex-col">
        ...
        {qrGenerado && (
          <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-3xl w-full max-w-sm text-center shadow-[0_0_50px_rgba(147,51,234,0.7)] relative overflow-hidden">
              ...
              <h2 className="text-4xl font-black text-black uppercase mb-1 tracking-tighter">¡CREADO!</h2><p className="text-purple-600 font-black uppercase text-sm mb-6">{sesionActiva?.nombre_fiesta}</p>
              <div className="bg-gray-100 p-4 rounded-2xl border border-dashed border-gray-300 mb-6"><p className="font-black text-2xl text-black uppercase">{qrGenerado.nombre}</p><p className="text-gray-600 font-bold text-lg mt-1">{qrGenerado.tipo_pase === 'vip' ? '👑 PASE VIP' : '🎫 PASE GENERAL'} ({qrGenerado.cantidad} pers)</p></div>
        ...
        {!sesionActiva ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center"><p className="text-6xl mb-4">🔒</p><h2 className="text-2xl font-bold text-red-400">En Espera</h2><p className="text-gray-400 mt-2">Un administrador debe abrir la caja adentro.</p></div>
        ) : (
          <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {(user.rol === 'admin' || user.rol === 'puerta') && (
              <div className="bg-gray-800 p-8 rounded-3xl border border-gray-700 shadow-2xl relative overflow-hidden h-fit">
                ...
                {user.rol === 'puerta' ? <p className="text-sm font-bold text-yellow-400 bg-yellow-900/30 p-2 rounded-lg mb-6 border border-yellow-800">🎟️ TU CUPO DISPONIBLE: {user.cupo_listas} Pases</p> : <p className="text-sm text-gray-400 mb-6">Genera invitaciones digitales para enviar por WhatsApp.</p>}
                <form onSubmit={generarQRLista} className="space-y-6 relative z-10">
                  <div><label className="text-xs text-gray-400 font-bold uppercase mb-2 block">Categoría de la Invitación</label><div className="flex space-x-2"><button type="button" onClick={() => setTipoPaseQr('general')} className={`flex-1 py-3 rounded-xl font-black uppercase transition ${tipoPaseQr === 'general' ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-400'}`}>Entrada General</button><button type="button" onClick={() => setTipoPaseQr('vip')} className={`flex-1 py-3 rounded-xl font-black uppercase transition ${tipoPaseQr === 'vip' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-400'}`}>VIP / Accesos</button></div></div>
            ...
            <div className={`space-y-6 ${user.rol === 'puerta' ? 'lg:col-span-2 max-w-xl mx-auto w-full' : ''}`}>
              <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 flex justify-between shadow-xl">
                <div className="text-center"><p className="text-xs text-gray-400 font-bold uppercase">Gratis (Adentro)</p><p className="text-4xl font-black text-green-400">{personasListaIngresadas}</p></div>
                <div className="text-center"><p className="text-xs text-gray-400 font-bold uppercase">QRs Activos</p><p className="text-4xl font-black text-yellow-400">{listasVip.filter(l => l.estado === 'pendiente').length}</p></div>
              </div>
              
              <div className="flex gap-3">
                <div className="relative flex-1"><input type="text" placeholder="Escribe DNI o Código..." className="w-full bg-gray-800 p-4 pl-12 rounded-xl border border-gray-700 font-black text-lg focus:outline-none focus:border-purple-500 shadow-inner" value={filtroQR} onChange={e => setFiltroQR(e.target.value)} /><span className="absolute left-4 top-4 text-xl">🔍</span></div>
                      ...
                      <div key={l.id} className={`p-5 rounded-2xl border-2 transition ${l.estado === 'ingresado' ? 'bg-green-900/10 border-green-900/50 opacity-50' : 'bg-gray-800 border-gray-600 shadow-lg'}`}>
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1">{l.tipo_pase} | Cód: <span className="text-yellow-400 font-bold">{l.codigo}</span></p>
                            <p className="text-sm text-blue-400 font-bold mt-2">Ingresados: {l.ingresados || 0} / {l.cantidad}</p>
                          </div>
                          {user.rol === 'admin' && <button onClick={() => descargarInvitacion(l)} className="bg-blue-600 hover:bg-blue-500 p-2 rounded-lg shadow transition">⬇️ QR</button>}
                        </div>
                        {l.estado === 'ingresado' ? (
                          <div className="mt-3 text-center bg-green-900/40 py-2 rounded-lg"><span className="text-green-500 font-black text-sm uppercase">✅ Completado</span></div>
                        ) : (
                          <button onClick={() => procesarEscaneoAutomatico(l.codigo)} className="w-full mt-3 bg-purple-600 hover:bg-purple-500 py-3 rounded-xl font-black text-sm uppercase shadow-lg active:scale-95 transition">Ingreso Parcial (+)</button>
                        )}
                      </div>
                    ))
                  }
  ...
  // VISTA: BOLETERIA (Usuario y Admin)
          ...
          <div className="max-w-md mx-auto w-full mt-4 bg-gray-800 p-8 rounded-3xl border border-gray-700 shadow-2xl">
            ...
            <form onSubmit={venderEntradas} className="space-y-6">
              ...
              <div>
                ...
                <div className="w-full bg-gray-900 p-4 rounded-xl font-black text-2xl text-center text-gray-300 border border-gray-700">${precioActualTaquilla}</div>
              </div>
              <div>
              ...
              <div className="bg-black/50 p-4 rounded-xl border border-gray-600 text-center"><span className="text-sm font-bold text-gray-400 block uppercase mb-1">Total a Cobrar</span><span className="text-5xl font-black text-green-400">${cantEntradas * precioActualTaquilla}</span></div>
              <div>
                ...
                <div key={p.id} className="bg-gray-800 p-6 rounded-3xl border border-gray-700 shadow-xl relative flex flex-col">
                  ...
                  <div className="bg-gray-700/50 p-3 rounded-xl flex-1 max-h-[200px] overflow-y-auto mb-4 border border-gray-600 space-y-2 custom-scrollbar">
                    {(!p.compras || p.compras.length === 0) ? <p className="text-gray-500 text-sm text-center mt-4 font-bold uppercase">Sin deudas.</p> : 
                      p.compras.map(c => (
                        <div key={c.id} className="flex justify-between items-center bg-gray-800 p-3 rounded-lg text-sm shadow border border-gray-700"><div className="flex-1 pr-2"><p className="font-bold text-white line-clamp-1">{c.producto}</p><p className="text-[10px] text-gray-400">{c.tipo==='bebida'?`${c.cantidad}x (Stock sumado)`:'Cargo extra'}</p></div><span className="font-black text-orange-400 text-lg">${(c.cantidad||1) * c.costo}</span></div>
                      ))
                    }
                  </div>
                  ...
                  <div className="flex space-x-2 mb-3"><button onClick={() => {setModalProv(p); setTipoProvReg('bebida');}} className="flex-1 bg-gray-600 hover:bg-gray-500 py-3 rounded-xl font-black text-xs uppercase shadow transition active:scale-95">+ Sumar</button><button onClick={() => aplicarDescuentoProv(p.id, p.descuento)} className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-black py-3 rounded-xl font-black text-xs uppercase shadow transition active:scale-95">🎁 Desc.</button></div>
