...
function App() {
  ...
  // Modal de Detalles ('entrada', 'salida', 'ventas_efectivo', 'ventas_transferencia')
  ...
  const eliminarProducto = async (id, nombre) => {
    if (window.confirm(`⚠️ PELIGRO: ¿Estás seguro de ELIMINAR "${nombre}" y todo su stock del menú? Esta acción no se puede deshacer.`)) {
      setLoading(true);
  ...
  const procesarCierre = async () => {
    if (!window.confirm('⚠️ ¿Estás seguro de CERRAR CAJA definitivamente y volver a cero? Imprime el reporte Z.')) return;
    setLoading(true);
  ...
  const RenderizarModalDetalles = () => {
    ...
    if (verDetalleModal === 'entrada') { titulo = 'Ingresos Extra'; datos = movsSesion.filter(m => m.tipo === 'entrada'); }
    if (verDetalleModal === 'salida') { titulo = 'Salidas y Gastos'; datos = movsSesion.filter(m => m.tipo === 'salida'); }
    if (verDetalleModal === 'ventas_efectivo') { titulo = 'Ventas en Efectivo'; datos = ventasSesion.filter(v => v.metodo_pago === 'efectivo'); esVenta = true; }
    if (verDetalleModal === 'ventas_transferencia') { titulo = 'Ventas por Transferencia'; datos = ventasSesion.filter(v => v.metodo_pago === 'transferencia'); esVenta = true; }

    return (
      <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
        <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 w-full max-w-lg max-h-[80vh] flex flex-col">
          <h2 className="text-xl font-black uppercase mb-4 text-purple-400 border-b border-gray-700 pb-2">{titulo}</h2>
          <div className="flex-1 overflow-y-auto space-y-3 pr-2">
            {datos.length === 0 ? <p className="text-gray-500 text-center py-4">No hay registros.</p> :
              datos.map(d => (
                <div key={d.id} className="bg-gray-700 p-3 rounded-lg flex justify-between items-center text-sm border border-gray-600">
                  <div className="flex-1 pr-2">
                    {esVenta ? (
                      <>
                        <p className="font-bold text-xs text-gray-300">Ticket #{d.id} - Cajero: {d.cajero}</p>
                        <p className="text-xs text-gray-400 italic line-clamp-1">{d.detalles.map(i => `${i.cantidad}x ${i.nombre}`).join(', ')}</p>
                      </>
                    ) : (
                      <>
                        <p className="font-bold text-sm text-white">{d.concepto}</p>
                        <p className="text-xs text-gray-400 uppercase">Vía: {d.metodo_pago}</p>
                      </>
                    )}
                  </div>
                  <span className={`font-black text-lg ${verDetalleModal === 'salida' ? 'text-red-400' : 'text-green-400'}`}>
                    ${esVenta ? d.total : d.monto}
                  </span>
                </div>
              ))
            }
        ...
        <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8">
          ...
          <div className="flex flex-col items-center justify-center mt-12">
            <div className="bg-gray-800 p-8 rounded-2xl border border-gray-700 w-full max-w-md text-center shadow-2xl">
              ...
              <p className="text-gray-400 text-sm mb-6">Inicia un turno para habilitar la caja y registrar las ventas.</p>
              <form onSubmit={abrirCaja} className="space-y-4">
                <input type="text" placeholder="Ej: Fiesta Halloween..." className="w-full bg-gray-700 p-4 rounded-xl font-black text-white text-center text-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition" value={nombreFiestaApertura} onChange={e => setNombreFiestaApertura(e.target.value)} required />
          ...
          <div className="space-y-6">
            <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 shadow-xl">
              ...
              <form onSubmit={registrarMovimiento} className="space-y-3">
                <select className="w-full bg-gray-700 p-3 rounded-lg focus:outline-none" value={movTipo} onChange={e => setMovTipo(e.target.value)}><option value="salida">🔴 Salida (Pago Staff, Hielo)</option><option value="entrada">🟢 Ingreso Extra (Fondo Caja)</option></select>
                ...
                <div className="flex space-x-2">
                  <input type="number" placeholder="Monto $" className="w-2/3 bg-gray-700 p-3 rounded-lg focus:outline-none font-bold" value={movMonto} onChange={e => setMovMonto(e.target.value)} required />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {bebidas.map(b => (
                <div key={b.id} className={`p-4 rounded-xl border relative ${b.stock < 10 ? 'bg-red-900/10 border-red-900' : 'bg-gray-700/30 border-gray-600'}`}>
                  <button onClick={() => eliminarProducto(b.id, b.nombre)} className="absolute top-2 right-2 text-gray-500 hover:text-red-500 text-lg transition">❌</button>
                  ...
                  <div className="flex space-x-2">
                    <button onClick={async () => { const n = prompt('Nuevo precio:', b.precio); if (n) { await supabase.from('bebidas').update({precio:Number(n)}).eq('id',b.id); cargarDatos();} }} className="flex-1 bg-gray-600 hover:bg-gray-500 py-2 rounded text-xs font-bold uppercase transition">Cambiar $</button>
                    <button onClick={async () => { const s = prompt('Sumar stock:', 10); if (s) { await supabase.from('bebidas').update({stock:b.stock+Number(s)}).eq('id',b.id); cargarDatos();} }} className="flex-1 bg-blue-600 hover:bg-blue-500 py-2 rounded text-xs font-bold uppercase transition">+ Stock</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex-1 p-2 lg:p-4 grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-6xl mx-auto w-full">
        <div className="lg:col-span-2 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 lg:gap-3">
            {bebidas.map((item) => (
              <button key={item.id} onClick={() => agregarAlCarrito(item)} className={`p-3 lg:p-4 rounded-xl border text-left flex flex-col justify-between transition active:scale-95 ${item.stock > 0 ? 'bg-gray-800 border-gray-700 hover:border-purple-500' : 'bg-gray-800/40 border-gray-800 opacity-50'}`}>
        ...
        <div className="bg-gray-800 rounded-2xl p-4 border border-gray-700 flex flex-col justify-between h-[450px] lg:h-auto shadow-2xl">
                ...
                {carrito.map((item) => (
                  <div key={item.id} className="flex items-center justify-between bg-gray-700/40 p-2 rounded-lg border border-gray-600">
                    ...
                    <div className="flex items-center space-x-1">
                      <button onClick={() => cambiarCantidad(item.id, -1)} className="bg-gray-600 w-8 h-8 rounded-lg font-black text-sm active:bg-gray-500">-</button>
          ...
          <div className="pt-2">
            ...
            <div className="flex space-x-2 mb-3">
              <button onClick={() => setMetodoPagoPOS('efectivo')} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition ${metodoPagoPOS === 'efectivo' ? 'bg-blue-600 text-white shadow-[0_0_10px_rgba(37,99,235,0.5)]' : 'bg-gray-700 text-gray-400 border border-gray-600'}`}>💵 Efectivo</button>
              <button onClick={() => setMetodoPagoPOS('transferencia')} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition ${metodoPagoPOS === 'transferencia' ? 'bg-purple-600 text-white shadow-[0_0_10px_rgba(147,51,234,0.5)]' : 'bg-gray-700 text-gray-400 border border-gray-600'}`}>📱 Transf</button>
            </div>
            <button onClick={procesarVenta} disabled={carrito.length === 0 || loading} className="w-full bg-green-600 hover:bg-green-500 disabled:bg-gray-700 text-white font-black py-4 rounded-xl shadow-[0_0_15px_rgba(34,197,94,0.3)] transition active:scale-95 text-lg uppercase tracking-widest">{loading ? '...' : 'COBRAR'}</button>
          </div>
        </div>
      </div>
    </div>
