...
function App() {
  ...
  const generarQRLista = async (e) => { 
    ...
    // Verificación de Cupo si es usuario puerta
    if (user.rol === 'puerta') {
      if (user.cupo_listas < cantLista) {
        return alert(`❌ CUPO INSUFICIENTE.\nTe quedan ${user.cupo_listas} lugares pero intentas meter a ${cantLista}. Pide más cupo al administrador.`);
      }
    }
    ...
    if (!error) { 
      if (user.rol === 'puerta') {
        const nuevoCupo = user.cupo_listas - cantLista;
        await supabase.from('cajeros').update({ cupo_listas: nuevoCupo }).eq('id', user.id);
      }
  ...
  const procesarEscaneoAutomatico = async (textoCodigo) => { 
    ...
    if (listaEncontrada.estado === 'ingresado') { alert(`⚠️ CÓDIGO COMPLETADO.\nYa entraron las ${listaEncontrada.cantidad} personas.`); setFiltroQR(textoCodigo); return; } 
      ...
      <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 lg:col-span-2 shadow-xl space-y-6">
            ...
            <form onSubmit={crearProducto} className="flex flex-col sm:flex-row gap-2 bg-gray-700/50 p-3 rounded-xl border border-gray-600">
  ...
  if (vista === 'puerta' || user.rol === 'puerta') {
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8">
        ...
        {!sesionActiva ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center"><p className="text-6xl mb-4">🔒</p><h2 className="text-2xl font-bold text-red-400">En Espera</h2></div>
        ) : (
          <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-6">
            
            {/* Solo mostramos el generador de QR a Admin o a Puerta SI TIENE CUPO */}
            <div className="bg-gray-800 p-8 rounded-3xl border border-gray-700 shadow-2xl relative overflow-hidden h-fit">
              ...
              <h2 className="text-2xl font-black uppercase text-purple-400 mb-2 flex items-center gap-2">📱 Generar Pase VIP</h2>
              {user.rol === 'puerta' ? (
                <p className="text-sm font-bold text-yellow-400 bg-yellow-900/30 p-2 rounded-lg mb-6 border border-yellow-800">🎟️ TU CUPO DISPONIBLE: {user.cupo_listas} Pases</p>
              ) : (
              ...
              <form onSubmit={generarQRLista} className="space-y-6 relative z-10">
                <div>
                  ...
                  <div className="flex space-x-2">
                    <button type="button" onClick={() => setTipoPaseQr('general')} className={`flex-1 py-3 rounded-xl font-black uppercase transition ${tipoPaseQr === 'general' ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-400'}`}>General</button>
                    <button type="button" onClick={() => setTipoPaseQr('vip')} className={`flex-1 py-3 rounded-xl font-black uppercase transition ${tipoPaseQr === 'vip' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-400'}`}>VIP</button>
                  </div>
                </div>
            ...
            {/* Zona de Escáner y Control de Accesos */}
