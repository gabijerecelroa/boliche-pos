...
function App() {
  ...
  const actualizarPreciosTaquilla = async (e) => {
    e.preventDefault(); setLoading(true);
    await supabase.from('sesiones').update({ precio_general: Number(precioG), precio_vip: Number(precioV) }).eq('id', sesionActiva.id);
    await cargarDatos(); setLoading(false); alert('✅ Precios actualizados en vivo para todas las cajas.');
  };
  ...
  const venderEntradas = async (e) => { 
    ...
    await supabase.from('puerta').insert([{ sesion_id: sesionActiva.id, tipo: 'venta', nombre: `Pulsera ${tipoEntradaVenta}`, cantidad: cantEntradas, precio_unitario: precioActualTaquilla, total, metodo_pago: pagoEntrada }]); 
  ...
  const descargarInvitacion = (qrData) => {
    ...
    ctx.fillStyle = '#10b981'; ctx.font = 'bold 45px sans-serif'; ctx.textAlign = 'center'; ctx.fillText(`FIESTA: ${sesionActiva?.nombre_fiesta?.toUpperCase() || ''}`, 540, 150);
    ctx.fillStyle = '#d8b4fe'; ctx.font = 'bold 80px sans-serif'; ctx.fillText('GJBROSS', 540, 250);
    ctx.fillStyle = '#ffffff'; ctx.font = 'bold 120px sans-serif'; ctx.fillText(`PASE ${qrData.tipo_pase?.toUpperCase() || 'VIP'}`, 540, 420);
    ctx.fillStyle = '#fbbf24'; ctx.font = '60px sans-serif'; ctx.fillText(qrData.nombre.toUpperCase(), 540, 600);
  ...
  const procesarEscaneoAutomatico = async (textoCodigo) => { 
    ...
    if (listaEncontrada.estado === 'ingresado') { alert(`⚠️ CÓDIGO COMPLETADO.\nYa entraron las ${listaEncontrada.cantidad} personas de este QR.`); setFiltroQR(textoCodigo); return; } 
  ...
  // Funciones Generales POS y Proveedores (Resumidas para Vistas)
  ...
  const procesarVenta = async () => { if (carrito.length === 0 || !sesionActiva) return; setLoading(true); const totalVenta = carrito.reduce((a, item) => a + (item.precio * item.cantidad), 0); const { data: ventaData, error } = await supabase.from('ventas').insert([{ cajero: user.usuario, total: totalVenta, detalles: carrito, metodo_pago: metodoPagoPOS, sesion_id: sesionActiva.id }]).select().single(); if (!error) { for (const item of carrito) await supabase.from('bebidas').update({ stock: item.stock - item.cantidad }).eq('id', item.id); setTicketActual({ tipo: 'venta', id: ventaData.id, fiesta: sesionActiva.nombre_fiesta, cajero: user.usuario, fecha: new Date().toLocaleTimeString(), items: [...carrito], total: totalVenta, metodo_pago: metodoPagoPOS }); setCarrito([]); cargarDatos(); } setLoading(false); };
  ...
  const barraHeader = (
    <header className="bg-gray-800 px-4 py-3 border-b border-gray-700 flex flex-col md:flex-row justify-between items-center mb-4 rounded-b-xl lg:rounded-xl gap-3 shadow-lg">
      <div className="flex flex-col items-center md:items-start w-full md:w-auto"><h1 className="text-xl font-black tracking-wider text-purple-400">GJBROSS <span className="text-white text-sm">POS</span></h1>{sesionActiva ? <p className="text-xs text-green-400 font-bold uppercase">🟢 {sesionActiva.nombre_fiesta}</p> : <p className="text-xs text-red-400 font-bold uppercase">🔴 CAJA CERRADA</p>}</div>
  ...
  if (ticketActual) return (<div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center"><div className="bg-white text-black p-6 rounded w-full max-w-sm text-center font-mono border border-gray-400"><h2 className="text-2xl font-black uppercase text-center">{ticketActual.tipo === 'cierre' ? 'REPORTE Z' : 'GJBROSS POS'}</h2><p className="text-sm font-bold text-center mt-1 bg-gray-200 py-1">{ticketActual.fiesta}</p>{ticketActual.tipo === 'venta' ? (<><div className="text-left text-xs mb-2 mt-4"><p><b>Ticket:</b> #{ticketActual.id}</p><p><b>Cajero:</b> {ticketActual.cajero}</p><p><b>Hora:</b> {ticketActual.fecha}</p></div><hr className="my-2 border-dashed border-gray-400" /><div className="text-left space-y-1">{ticketActual.items.map((it) => (<div key={it.id} className="flex justify-between text-sm"><span>{it.cantidad}x {it.nombre}</span><span>${it.precio * it.cantidad}</span></div>))}</div><hr className="my-2 border-dashed border-gray-400" /><h3 className="text-2xl font-black text-right">TOTAL: ${ticketActual.total}</h3><p className="text-xs text-center mt-2 font-bold bg-black text-white py-1 uppercase border border-dashed">METODO: {ticketActual.metodo_pago}</p></>) : (<><div className="text-left text-xs space-y-1 mt-4 mb-2"><p><b>Cierre:</b> {ticketActual.fecha} - {ticketActual.hora}</p><p><b>Resp:</b> {ticketActual.responsable}</p></div><hr className="border-black my-2" /><h4 className="font-bold text-xs text-left uppercase mb-1">Métricas de Puerta</h4><div className="text-left text-xs space-y-1 bg-gray-100 p-2 border border-dashed"><div className="flex justify-between"><span>Vendidas (Generales):</span><span className="font-bold">{ticketActual.cant_generales} pers.</span></div><div className="flex justify-between"><span>Vendidas (VIPs):</span><span className="font-bold">{ticketActual.cant_vips} pers.</span></div><div className="flex justify-between text-red-600"><span>Listas Gratis Ingresadas:</span><span className="font-bold">{ticketActual.personas_lista} pers.</span></div><div className="flex justify-between text-sm font-black mt-1"><span>TOTAL ADENTRO:</span><span>{ticketActual.personas_vendidas + ticketActual.personas_lista} pers.</span></div></div><hr className="border-black my-2" /><div className="text-left text-xs space-y-1"><div className="flex justify-between font-bold"><span>Total Ventas (Barra+Puerta):</span><span>${ticketActual.ventas_efectivo + ticketActual.ventas_transf + ticketActual.puerta_efectivo + ticketActual.puerta_transf}</span></div><div className="flex justify-between text-green-600"><span>Entradas de plata extra:</span><span>+${ticketActual.entradas_efec + ticketActual.entradas_transf}</span></div><div className="flex justify-between text-red-600"><span>Pagos/Salidas de caja:</span><span>-${ticketActual.salidas_efec + ticketActual.salidas_transf}</span></div></div><hr className="my-3 border-black" /><div className="bg-black text-white p-2 text-left text-sm space-y-1"><div className="flex justify-between text-gray-300"><span>A Rendir EFECTIVO:</span><span>${ticketActual.recaudacion_efectivo}</span></div><div className="flex justify-between text-gray-300"><span>A Rendir BANCO/MP:</span><span>${ticketActual.recaudacion_transf}</span></div><hr className="border-gray-500 my-1"/><div className="flex justify-between"><span className="font-bold uppercase">Recaudación Neta:</span><span className="text-xl font-black">${ticketActual.recaudacion_efectivo + ticketActual.recaudacion_transf}</span></div></div></>)}</div><div className="mt-6 flex space-x-4"><button onClick={() => window.print()} className="bg-green-600 px-6 py-3 rounded-lg font-black uppercase text-sm">🖨️ Imprimir Ticket</button><button onClick={() => setTicketActual(null)} className="bg-purple-600 px-6 py-3 rounded-lg font-black uppercase text-sm">➡️ Continuar</button></div></div>);

  // COMPONENTES REUTILIZABLES PARA FORMULARIOS DE VENTA TAQUILLA (Blindados)
  const renderBoleteriaVenta = () => (
    <div className="max-w-md mx-auto w-full mt-4 bg-gray-800 p-8 rounded-3xl border border-gray-700 shadow-2xl">
      ...
      <form onSubmit={venderEntradas} className="space-y-6">
        ...
        <div>
          <label className="text-xs text-gray-400 font-bold uppercase mb-2 block">Precio Unitario Oficial ($)</label>
          <div className="w-full bg-gray-900 p-4 rounded-xl font-black text-2xl text-center text-gray-300 border border-gray-700">
            ${precioActualTaquilla}
          </div>
        </div>
        <div>
        ...
        <div className="bg-black/50 p-4 rounded-xl border border-gray-600 text-center">
          ...
          <span className="text-5xl font-black text-green-400">${cantEntradas * precioActualTaquilla}</span>
        </div>
        <div>
  ...
  // VISTA EXCLUSIVA: USUARIO PUERTA
  if (user.rol === 'puerta') {
      ...
      <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8 flex flex-col">
        {mostrarEscaner && (
          <div className="fixed inset-0 bg-black z-[100] flex flex-col">
            <div className="bg-gray-900 p-4 flex justify-between items-center border-b border-gray-700 pt-8"><h2 className="text-xl font-black text-purple-400 tracking-widest">ESCANEAR PASE VIP</h2><button onClick={() => setMostrarEscaner(false)} className="text-red-500 font-black text-lg bg-gray-800 px-4 py-2 rounded-lg">CERRAR ✖</button></div>
            <div className="flex-1 w-full flex items-center justify-center bg-black p-4"><div className="w-full max-w-sm rounded-3xl overflow-hidden border-4 border-purple-500 shadow-[0_0_40px_rgba(147,51,234,0.4)] relative"><Scanner onScan={(r) => { if(!r) return; const txt = Array.isArray(r)?r[0].rawValue:(r.text||r); if(txt) procesarEscaneoAutomatico(txt); }} onError={console.log} /><div className="absolute inset-0 border-[40px] border-black/40 pointer-events-none"></div></div></div>
            <div className="p-8 bg-gray-900 text-center pb-12"><p className="text-gray-400 text-sm font-bold uppercase tracking-widest">Apunta la cámara al código</p></div>
          </div>
        )}
        <header className="bg-gray-800 p-4 rounded-xl mb-6 flex justify-between items-center border border-purple-500/30 shadow-lg"><div><h1 className="text-xl font-black text-purple-400">CONTROL PUERTA</h1><p className="text-xs text-green-400 uppercase font-bold">{sesionActiva ? sesionActiva.nombre_fiesta : 'Caja Cerrada'}</p></div><button onClick={() => {setUser(null); setVista('login');}} className="bg-red-900 px-4 py-2 rounded-lg font-bold text-xs uppercase shadow">Salir</button></header>
        {!sesionActiva ? <div className="flex-1 flex flex-col items-center justify-center text-center"><p className="text-6xl mb-4">🔒</p><h2 className="text-2xl font-bold text-red-400">En Espera</h2><p className="text-gray-400 mt-2">Un administrador debe abrir la caja adentro.</p></div> : (
          <div className="max-w-xl mx-auto w-full space-y-6">
            <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 flex justify-between shadow-xl"><div className="text-center"><p className="text-xs text-gray-400 font-bold uppercase">Ingresados (Adentro)</p><p className="text-4xl font-black text-green-400">{personasListaIngresadas}</p></div><div className="text-center"><p className="text-xs text-gray-400 font-bold uppercase">Listas Pendientes</p><p className="text-4xl font-black text-yellow-400">{listasVip.filter(l => l.estado === 'pendiente').length}</p></div></div>
            <div className="flex gap-3"><div className="relative flex-1"><input type="text" placeholder="Escribe DNI o Código..." className="w-full bg-gray-800 p-4 pl-12 rounded-xl border border-gray-700 font-black text-lg focus:outline-none focus:border-purple-500 shadow-inner" value={filtroQR} onChange={e => setFiltroQR(e.target.value)} /><span className="absolute left-4 top-4 text-xl">🔍</span></div><button onClick={() => setMostrarEscaner(true)} className="bg-purple-600 hover:bg-purple-500 px-6 rounded-xl font-black text-3xl shadow-[0_0_15px_rgba(147,51,234,0.5)] transition active:scale-95 flex items-center justify-center">📷</button></div>
            <div className="space-y-3 max-h-[50vh] overflow-y-auto custom-scrollbar pb-10">{listasFiltradas.length === 0 ? <p className="text-center text-gray-500 py-10 font-bold uppercase">No se encontraron resultados.</p> : listasFiltradas.map(l => (<div key={l.id} className={`p-5 rounded-2xl border-2 flex justify-between items-center transition ${l.estado === 'ingresado' ? 'bg-green-900/10 border-green-900/50 opacity-40' : 'bg-gray-800 border-gray-600 shadow-lg'}`}><div><p className="font-black text-xl uppercase text-white">{l.nombre}</p><p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1">{l.tipo_pase} | Cód: <span className="text-yellow-400 font-bold">{l.codigo}</span></p><p className="text-sm text-blue-400 font-bold mt-2">Ingresados: {l.ingresados || 0} / {l.cantidad}</p></div>{l.estado === 'ingresado' ? <span className="text-green-500 font-black text-sm uppercase bg-green-900/40 px-3 py-2 rounded-lg">✅ Adentro</span> : <button onClick={() => procesarEscaneoAutomatico(l.codigo)} className="bg-purple-600 hover:bg-purple-500 px-5 py-4 rounded-xl font-black text-sm uppercase shadow-lg active:scale-95 transition">Dar Ingreso (+)</button>}</div>))}</div>
          </div>
        )}
      </div>
    );
  ...
  // VISTA: BOLETERIA
  if (vista === 'boleteria') return <div className="min-h-screen bg-gray-900 text-white p-4 flex flex-col">{barraHeader}{!sesionActiva ? <div className="flex-1 flex flex-col items-center justify-center text-center"><p className="text-6xl mb-4">🔒</p><h2 className="text-2xl font-bold text-red-400">Caja Cerrada</h2></div> : renderBoleteriaVenta()}</div>;
      ...
      <div className="min-h-screen bg-gray-900 text-white p-4 lg:p-8 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="space-y-6">
                ...
                {listasVip.filter(l => l.ingresados > 0).length === 0 ? <p className="text-gray-500 text-sm">Nadie ha ingresado por QR aún.</p> :
                  listasVip.filter(l => l.ingresados > 0).map(l => (
                    <div key={l.id} className="flex justify-between items-center bg-gray-700/50 p-3 rounded-lg border border-gray-600">
                      <span className="font-bold text-white text-sm">{l.nombre}</span><span className="font-black text-green-400 text-xs">+{l.ingresados} Adentro</span>
                    </div>
                  ))
                }
              </div>
            ...
            <div className="bg-gray-800 p-6 rounded-2xl border border-gray-700 shadow-xl">
              <h2 className="text-lg font-black uppercase text-indigo-400 mb-4 flex items-center">🎟️ Precios Taquilla</h2>
              <form onSubmit={actualizarPreciosTaquilla} className="space-y-3">
                <div className="flex justify-between items-center bg-gray-700 p-3 rounded-lg"><span className="font-bold text-sm">General $</span><input type="number" className="bg-gray-800 p-2 rounded text-white font-black text-right w-24" value={precioG} onChange={e=>setPrecioG(e.target.value)} required /></div>
                <div className="flex justify-between items-center bg-gray-700 p-3 rounded-lg"><span className="font-bold text-sm">VIP $</span><input type="number" className="bg-gray-800 p-2 rounded text-white font-black text-right w-24" value={precioV} onChange={e=>setPrecioV(e.target.value)} required /></div>
                <button type="submit" disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-500 py-3 rounded-lg font-black uppercase text-sm">Fijar Precios</button>
              </form>
            </div>
