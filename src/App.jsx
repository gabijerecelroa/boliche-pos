...
function App() {
  ...
  const abrirCaja = async (e) => { e.preventDefault(); if (!nombreFiestaApertura) return; setLoading(true); await supabase.from('sesiones').insert([{ nombre_fiesta: nombreFiestaApertura, abierta_por: user.usuario }]); await cargarDatos(); setLoading(false); setVista('admin'); };
  const actualizarPreciosTaquilla = async (e) => { e.preventDefault(); setLoading(true); await supabase.from('sesiones').update({ precio_general: Number(precioG), precio_vip: Number(precioV) }).eq('id', sesionActiva.id); await cargarDatos(); setLoading(false); alert('✅ Precios de taquilla actualizados.'); };
  const crearStaff = async (e) => { e.preventDefault(); setLoading(true); await supabase.from('cajeros').insert([{ usuario: nuevoStaffUser, password: nuevoStaffPass, rol: nuevoStaffRol, cupo_listas: nuevoStaffRol === 'puerta' ? Number(nuevoStaffCupo) : 0 }]); setNuevoStaffUser(''); setNuevoStaffPass(''); setNuevoStaffCupo(0); await cargarDatos(); setLoading(false); alert('✅ Empleado creado exitosamente.'); };
  const eliminarStaff = async (id, nombre) => { if (window.confirm(`¿Seguro que deseas ELIMINAR al usuario "${nombre}"?`)) { setLoading(true); await supabase.from('cajeros').delete().eq('id', id); await cargarDatos(); setLoading(false); } };
  const sumarCupoStaff = async (id, cupoActual, nombre) => { const sumar = prompt(`¿Cuántos pases VIP extras le vas a sumar a ${nombre}?`); if (sumar && !isNaN(sumar)) { await supabase.from('cajeros').update({ cupo_listas: cupoActual + Number(sumar) }).eq('id', id); cargarDatos(); } };

  const venderEntradas = async (e) => { e.preventDefault(); if (!sesionActiva || cantEntradas < 1) return; setLoading(true); const total = cantEntradas * precioActualTaquilla; await supabase.from('puerta').insert([{ sesion_id: sesionActiva.id, tipo: 'venta', nombre: `Pulsera ${tipoEntradaVenta}`, cantidad: cantEntradas, precio_unitario: precioActualTaquilla, total, metodo_pago: pagoEntrada }]); setCantEntradas(1); await cargarDatos(); setLoading(false); alert(`✅ Venta Exitosa`); };
  ...
  const descargarInvitacion = (qrData) => { const canvas = document.createElement('canvas'); canvas.width = 1080; canvas.height = 1920; const ctx = canvas.getContext('2d'); const grad = ctx.createLinearGradient(0, 0, 1080, 1920); grad.addColorStop(0, '#0f0c29'); grad.addColorStop(0.5, '#302b63'); grad.addColorStop(1, '#24243e'); ctx.fillStyle = grad; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = '#a855f7'; ctx.lineWidth = 15; ctx.strokeRect(50, 50, 980, 1820); ctx.fillStyle = '#10b981'; ctx.font = 'bold 45px sans-serif'; ctx.textAlign = 'center'; ctx.fillText(`FIESTA: ${sesionActiva?.nombre_fiesta?.toUpperCase() || ''}`, 540, 150); ctx.fillStyle = '#d8b4fe'; ctx.font = 'bold 80px sans-serif'; ctx.fillText('GJBROSS', 540, 250); ctx.fillStyle = '#ffffff'; ctx.font = 'bold 120px sans-serif'; ctx.fillText(`PASE ${qrData.tipo_pase?.toUpperCase() || 'VIP'}`, 540, 420); ctx.fillStyle = '#fbbf24'; ctx.font = '60px sans-serif'; ctx.fillText(qrData.nombre.toUpperCase(), 540, 600); ctx.fillStyle = '#9ca3af'; ctx.font = '40px sans-serif'; ctx.fillText(`Válido para ${qrData.cantidad} personas`, 540, 680); const img = new Image(); img.crossOrigin = 'Anonymous'; img.onload = () => { ctx.fillStyle = '#ffffff'; ctx.fillRect(270, 800, 540, 540); ctx.drawImage(img, 290, 820, 500, 500); ctx.fillStyle = '#fbbf24'; ctx.font = 'bold 60px sans-serif'; ctx.fillText(qrData.codigo, 540, 1500); ctx.fillStyle = '#9ca3af'; ctx.font = '35px sans-serif'; ctx.fillText('Presenta este código en la puerta', 540, 1750); const link = document.createElement('a'); link.download = `Invitacion_${qrData.tipo_pase||'vip'}_${qrData.nombre}.png`; link.href = canvas.toDataURL('image/png'); link.click(); }; img.src = `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${qrData.codigo}&color=000000&bgcolor=FFFFFF`; };
  const procesarEscaneoAutomatico = async (textoCodigo) => { setMostrarEscaner(false); const listaEncontrada = listasVip.find(l => l.codigo.toUpperCase() === textoCodigo.toUpperCase()); if (!listaEncontrada) return alert('❌ CÓDIGO INVÁLIDO O INEXISTENTE.'); if (listaEncontrada.estado === 'ingresado') { alert(`⚠️ CÓDIGO COMPLETADO.\nYa entraron las ${listaEncontrada.cantidad} personas de este QR.`); setFiltroQR(textoCodigo); return; } const yaIngresados = listaEncontrada.ingresados || 0; const disponibles = listaEncontrada.cantidad - yaIngresados; const cantIngresarStr = prompt(`🎟️ PASE: ${listaEncontrada.nombre}\nQuedan disponibles: ${disponibles} (de ${listaEncontrada.cantidad}).\n¿Cuántos ingresan AHORA MISMO?`, disponibles); if (cantIngresarStr === null) return; const cantIngresar = Number(cantIngresarStr); if (isNaN(cantIngresar) || cantIngresar <= 0 || cantIngresar > disponibles) { return alert(`❌ Cantidad inválida.`); } setLoading(true); const nuevosIngresados = yaIngresados + cantIngresar; const nuevoEstado = nuevosIngresados >= listaEncontrada.cantidad ? 'ingresado' : 'pendiente'; await supabase.from('listas_vip').update({ ingresados: nuevosIngresados, estado: nuevoEstado }).eq('id', listaEncontrada.id); await supabase.from('puerta').insert([{ sesion_id: sesionActiva.id, tipo: 'lista', nombre: `Lista ${listaEncontrada.tipo_pase?.toUpperCase()||'VIP'} - ${listaEncontrada.nombre}`, cantidad: cantIngresar, precio_unitario: 0, total: 0 }]); setFiltroQR(''); await cargarDatos(); setLoading(false); alert(`✅ ACCESO PERMITIDO\nVIP: ${listaEncontrada.nombre}\nPASAN AHORA: ${cantIngresar}\nFaltan llegar: ${listaEncontrada.cantidad - nuevosIngresados}`); };
  ...
  // VISTA TICKET OPTIMIZADA PARA IMPRESORA TERMICA + AUTO CIERRE
  if (ticketActual) {
    return (
          ...
          {ticketActual.tipo === 'venta' ? (
            ...
            <>
              ...
              <div className="text-left text-xs space-y-1 print:space-y-0">
                ...
                <div className="flex justify-between text-green-600 print:text-black"><span>Entradas plata extra:</span><span>+${ticketActual.entradas_efec + ticketActual.entradas_transf}</span></div>
                <div className="flex justify-between text-red-600 print:text-black"><span>Pagos/Salidas caja:</span><span>-${ticketActual.salidas_efec + ticketActual.salidas_transf}</span></div>
              </div>
              ...
              <div className="bg-black text-white p-2 text-left text-sm space-y-1 print:bg-white print:text-black print:p-1 print:space-y-0">
                <div className="flex justify-between text-gray-300 print:text-black"><span>A Rendir EFECTIVO:</span><span>${ticketActual.recaudacion_efectivo}</span></div>
                <div className="flex justify-between text-gray-300 print:text-black"><span>A Rendir BANCO/MP:</span><span>${ticketActual.recaudacion_transf}</span></div>
                <hr className="border-gray-500 my-1 print:border-black"/>
                <div className="flex justify-between"><span className="font-bold uppercase">Recaudación Neta:</span><span className="text-xl font-black print:text-lg">${ticketActual.recaudacion_efectivo + ticketActual.recaudacion_transf}</span></div>
              </div>
            </>
          )}
        </div>
        <div className="mt-6 flex space-x-4 print:hidden">
          {/* EL TRUCO ESTÁ ACÁ: Imprime y automáticamente cierra el ticket visualmente */}
          ...
          <button onClick={() => setTicketActual(null)} className="bg-purple-600 px-6 py-3 rounded-lg font-black uppercase text-sm">➡️ Continuar (Sin Imprimir)</button>
        </div>
      </div>
    );
  }
